#' The CPIAggr <- function(fvcAppLanguage) ...
#' - Receives a language argument
#' - Creates global variables and updates loaded dataframes
#' - Downloads all data from CODR tables 18100004 and 18100007
...
#' - Defines the server function, which receives user input and displays results
#'	- Calls shinyApp(ui, server)
#'	@param fvcAppLanguage The dusokay language, 'en' or 'fr'. All other values default to 'en'.
#' @export
CPIAggr <- function(fvcAppLanguage){
...
if (missing(fvcAppLanguage)){
  cAppLanguage <<- "English"
} else if (tolower(fvcAppLanguage) == "fr"){
  cAppLanguage <<- "French"
}
